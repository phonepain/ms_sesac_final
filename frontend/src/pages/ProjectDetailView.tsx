...
import type { Project, SeverityType } from '../types';
...
import type { Contradiction } from '../components/project/ContradictionCard';
import { AI, Se } from '../components/common/Icons';
...
export default function ProjectDetailView({
  proj, tab, setTab, staged, onStageFix, onUnstageFix, onPushFixes, onClearStaged, onAnalyze, showAi, setShowAi
        ...
        <div className="overflow-auto flex flex-col gap-3 pr-2 scrollbar-thin">
          ...
          {tab === "contradictions" && (
            <>
              ...
              <div className="flex gap-1 flex-wrap mb-1">
                {(['all', 'critical', 'warning', 'info'] as const).map(f => {
                  ...
                  return (
                      ...
                      className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold transition-all ${
                        isActive 
                          ? (f === 'all' 
                              ...
                              : `text-[${sv?.tx}] bg-[${sv?.bg}] border border-[${sv?.bd}]`) 
                    ...
                    >
                      {f === 'all' ? '전체' : (f === 'critical' ? '심각' : f === 'warning' ? '주의' : '참고')}
              ...
              {filtered.length === 0 && (
                <div className="text-center py-8 text-[#3f3f46] text-[11px]">
                  모순이 없습니다
                </div>
              )}
            </>
                  ...
                  <div className={`rounded-xl p-2.5 border ${
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        {i === 0 && (
                          <span className="text-[7px] bg-[rgba(16,185,129,0.08)] text-[#34d399] px-1 py-0.5 rounded-sm">
                            최신
                          </span>
                        )}
                        {v.fx > 0 && (
                    ...
                    <div className="flex gap-1 mt-1.5">
                      <button className="text-[8px] text-[#52525b] px-1.5 py-0.5 rounded-[3px] border border-[rgba(63,63,70,0.1)] hover:text-[#e4e4e7] hover:border-[#52525b] transition-colors">
                        원고 보기
                      </button>
                      {i > 0 && (
                        <button className="text-[8px] text-[#52525b] px-1.5 py-0.5 rounded-[3px] border border-[rgba(63,63,70,0.1)] hover:text-[#e4e4e7] hover:border-[#52525b] transition-colors">
